class StackedAreaChart {
    ...
    initVis() {
        ...
        vis.xAxis = d3.axisBottom(x).ticks(d3.count(vis.data, function(d) { return d.year; }) /6)
    ...
    updateVis() { 
        ...
        console.log('Creating stack')
        vis.stack.value((d, key) => d[1][key].stat)
        console.log('vis stack val: ', vis.stack.value((d, key) => d[1][key].stat))
        vis.stackedData = vis.stack(vis.groupedData);
        console.log('Stacked data: ', vis.stackedData)

        console.log('Defining area')
        vis.area = d3.area()
        ...
        console.log('Setting scale input domains')
        vis.xScale.domain(d3.extent(vis.data, (d) => d.year));
