class StackedAreaChart2 {
  ...
  initVis() {
    ...
    vis.colorScale = d3.scaleOrdinal().range(["#6080b5", "#5a9866", "#f7dc7a"]);
  ...
  updateVis() {
    ...
    if (vis.config.displayType == "absolute") {
      vis.yScale.domain([
        0,
        d3.max(vis.stackedData[vis.stackedData.length - 1], (d) => d[1])
      ]);
  ...
  renderVis() {
    ...
    vis.axisTitle.style(
      "display",
      vis.config.displayType == "absolute" ? "block" : "none"
    );
