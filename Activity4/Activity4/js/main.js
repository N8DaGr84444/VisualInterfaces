...
let years = [];
 ...
 d3.csv('data/disasters.csv') //ASYNCHRONOUS LOADING
  .then(data => {
	...
	function getCircleFill(cat) {
		if (cat == "tropical-cyclone") {
			return "#081d58";
		} else if (cat == "drought-wildfire") {
			return "#ffffd9";
		} else if (cat == "severe-storm") {
			return "#c7e9b4";
		} else if (cat == "winter-storm-freeze") {
			return "#081d58";
	...
	svg.selectAll("circle")
		...
		.append("circle")
			...
			.attr("fill", (d) => getCircleFill(d.category))
			.attr("stroke", "gray")
			.attr("stroke-width", 2);
